...
Scoreboard::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
    ...
    if (show) {
        ...
        painter->drawPixmap(2 + networkLogoWidthOffset, TOP_BAR_HEIGHT + networkLogoHeightOffset, nchctv);
        ...
        if (showPP) {
            ...
            if(awayPP) {
                ...
                if (useClock)
                    painter->drawText(V_TEAM_BOX_STARTX, TOP_BAR_HEIGHT + SCOREBOARD_HEIGHT, TEAM_NAME_WIDTH - 14, PP_BAR_HEIGHT,
            ...
            //Home ppbar
                ...
                if (useClock)
                    painter->drawText(H_TEAM_BOX_STARTX + 4, TOP_BAR_HEIGHT + SCOREBOARD_HEIGHT, TEAM_NAME_WIDTH - 14, PP_BAR_HEIGHT,
                                      Qt::AlignRight | Qt::AlignVCenter, serialPP? serialPpClock : ppClock->toStringPP());
            }
            //Neutral
            else if (neutralPP){
...
Scoreboard::prepareColor() {
   ...
   // homeGradient.setColorAt(.6, homeColor);
    ...
    if (!Globals::onTv) {
        mainGradient.setColorAt(0, Globals::nchcBrandingTop);
        mainGradient.setColorAt(1, Globals::nchcBrandingBottom);
        neutralPPGradient.setColorAt(0, Globals::nchcBrandingTop);
        neutralPPGradient.setColorAt(1, Globals::nchcBrandingBottom);
    } else {
        mainGradient.setColorAt(0, Globals::tvBrandingTop);
        mainGradient.setColorAt(1, Globals::tvBrandingBottom);
        neutralPPGradient.setColorAt(0, Globals::tvBrandingTop);
        neutralPPGradient.setColorAt(1, Globals::tvBrandingBottom);
    }
...
Scoreboard::getClockRectCoords() {
    return QRect(x() + CLOCK_FIELD_X, y() + TOP_BAR_HEIGHT, CLOCK_FIELD_WIDTH, SCOREBOARD_HEIGHT);
 }
...
void Scoreboard::updateSog(bool isHome, int sog)
{
    ...
    scene()->update(x() + V_TEAM_BOX_STARTX + TEAM_NAME_WIDTH, y() +TOP_BAR_HEIGHT + SCOREBOARD_HEIGHT, TEAM_WIDTH - TEAM_NAME_WIDTH, PP_BAR_HEIGHT);
    scene()->update(x() + H_TEAM_BOX_STARTX + TEAM_NAME_WIDTH, y() +TOP_BAR_HEIGHT + SCOREBOARD_HEIGHT, TEAM_WIDTH - TEAM_NAME_WIDTH, PP_BAR_HEIGHT);
}
